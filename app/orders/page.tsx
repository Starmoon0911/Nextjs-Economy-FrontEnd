...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        const { data } = await axios.get(`/api/v1/order/getUserOrder?id=${userId}`);
        if (data.message === 'success') {
          ...
          const userMap = userResults.reduce((acc: Record<string, string>, cur) => {
          ...
          const productMap = productResults.reduce((acc: Record<string, string>, cur) => {
  ...
  const CancelOrder = async (orderId: string) => {
    try {
      ...
      await axios.post(`/api/v1/order/delete`, { OrderId: orderId }, {
        headers: {
